% Code to reproduce reward rates for each patch as a function of free or forced choice conditions (Figure 4)
...
for isub=1:nsubs
    ...
    for cond=1:2
        ...
        switch cond
            case 1
                forced_slowPatRew=vertcat(forced_slowPatRew,mean(dat.actual_rew_gain(idx(sl_idx)-2)));
                forced_midPatRew=vertcat(forced_midPatRew,mean(dat.actual_rew_gain(idx(mid_idx)-2)));
                forced_fastPatRew=vertcat(forced_fastPatRew,mean(dat.actual_rew_gain(idx(fst_idx)-2)));
                
            case 2
                free_slowPatRew=vertcat(free_slowPatRew,mean(dat.actual_rew_gain(idx(sl_idx)-2)));
                free_midPatRew=vertcat(free_midPatRew,mean(dat.actual_rew_gain(idx(mid_idx)-2)));
                free_fastPatRew=vertcat(free_fastPatRew,mean(dat.actual_rew_gain(idx(fst_idx)-2))); 
        end
    end
...
analysisname='Fig4_rewB4Leaving';
