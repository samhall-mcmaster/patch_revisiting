% Code to reproduce arrival reward, as a function of the patch being left, under free or forced choice conditions (Figure 5B)
...
for isub=1:nsubs
    ...
    for cond=1:2
        ...
        for itrl=1:size(idx,1)
            ...
            for m=1:10
                ...
                if dat.currentPatchReplenishRate(idx(itrl)+m)~=t(c,1) ...
                        & strcmp(dat.exploit_phase_key(idx(itrl)+m),'space')==1
                    t(c,2)=dat.actual_rew_gain(idx(itrl)+m);
                    break
                end 
            end
        end
        ...
        % get count for each patch
        sl_prop =  mean(t( find(t(:,1)==0.0500) ,2));
        mid_prop = mean(t(find(t(:,1)==0.1000)  ,2));
        fst_prop = mean(t(find(t(:,1)==0.1500)  ,2));
...
analysisname='Fig5B_arrivalFromPatch';
